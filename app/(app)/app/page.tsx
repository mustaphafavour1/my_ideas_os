...
async function getDashboardData(range: string, rangeFrom?: string, rangeTo?: string) {
  ...
  const supabase = demo ? null : createServiceClient();

  const [{ data: ideas }, { data: syncLog }, { data: inboxItems }, { data: userStats }] = demo
    ? [{ data: DEMO_IDEAS }, { data: null }, { data: [] }, { data: DEMO_USER_STATS }]
    : await Promise.all([
  ...
  const effectiveStats = demo ? DEMO_USER_STATS : ((userStats as UserStats | null) ?? (allIdeas.some((i) => i.source_ref === 'demo_mode') ? DEMO_USER_STATS : null));
