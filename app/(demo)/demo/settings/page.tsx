...
const SECTION_META: Record<SectionId, SectionMeta> = {
  ...
  attention:   {
    label: 'Needs Attention & AI Suggestions', desc: '',
    subToggles: [
...
function Toggle({ on, onChange }: { on: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      aria-checked={on}
    >
      <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-[#0A0A0F] transition-transform ${on ? 'translate-x-[18px]' : 'translate-x-[3px]'}`} />
    </button>
  );
}
            ...
            {FONT_OPTIONS.map(({ key, label, desc, sample }) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  {settings.font === key && <span className="w-1.5 h-1.5 rounded-full bg-[#F7C948]" />}
                </div>
              </button>
            ))}
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    {meta.toggle && (
                      <Toggle on={settings.dashboard[meta.toggle]} onChange={(v) => setDash(meta.toggle!, v)} />
                    )}
                  </div>
                  {meta.subToggles && (
