...
const QUICK_PROMPTS = [
  ...
  'Are there any patterns across my ideas?',
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            disabled={loading}
            className="flex-1 bg-[#0A0A0F] border border-[#1E1E2E] rounded-lg px-3 py-2 text-[12px] text-[#E0E0EA] placeholder-[#3A3A55] focus:outline-none focus:border-[#F7C948]/30 disabled:opacity-50 transition-colors"
          />
          <button
            onClick={() => ask(input)}
            disabled={loading || !input.trim()}
            className="px-4 py-2 bg-[#F7C948] text-[#0A0A0F] text-[11px] font-mono font-semibold rounded-lg hover:bg-[#E6B830] disabled:opacity-40 disabled:cursor-not-allowed transition-colors shrink-0"
          >
            Ask
          </button>
        </div>
